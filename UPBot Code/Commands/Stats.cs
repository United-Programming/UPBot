...
public class Stats : BaseCommandModule {
  ...
  [Command("stats")]
  [Description("Provides server stats for the 4 most used channels (Unity, CSharp, Help 1, and Help 2")]
  public async Task DoStats(CommandContext ctx) {
    await GenerateStats(ctx, null, 100);
  }
  [Command("stats")]
  [Description("Provides server stats for a specific channel")]
  public async Task DoStats(CommandContext ctx, DiscordChannel channel) {
    await GenerateStats(ctx, channel, 100);
  }
  [Command("stats")]
  [Description("Provides server stats, for a specified chanenl and with the required number of messages")]
  public async Task DoStats(CommandContext ctx, DiscordChannel channel, int numMessages) {
    await GenerateStats(ctx, channel, numMessages);
  }
  [Command("stats")]
  [Description("Provides server stats, for a specified chanenl and with the required number of messages")]
  public async Task DoStats(CommandContext ctx, int numMessages, DiscordChannel channel) {
    await GenerateStats(ctx, channel, numMessages);
  }
  [Command("stats")]
  [Description("Provides server stats for the 4 most used channels, with the required number of messages")]
  public async Task DoStats(CommandContext ctx, int numMessages) {
  ...
  const ulong idUnityChannel = 830904407540367441ul;
  const ulong idCSharpChannel = 830904726375628850ul;
  const ulong idHelp1Channel = 830921265648631878ul;
  const ulong idHelp2Channel = 830921315657449472ul;
}
