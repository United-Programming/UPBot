...
public class SlashTags : ApplicationCommandModule
{
    ...
    public async Task TagAddCommand(InteractionContext ctx, [Option("tagname", "Tag to be added")] string tagname)
    {
        ...
        try
        {
            ...
            TagBase tagBase = new TagBase(ctx.Guild.Id, tagname, answer.Result.Content, "Unknown", 35, DateTime.Now); // creating line inside of database
        ...
        try
        {
            ...
            else
            {
                ...
                foreach (TagBase tag in Setup.Tags[ctx.Guild.Id])
                {
                    ...
                    if (count < Setup.Tags[ctx.Guild.Id].Count - 1) result += ", \n";
    ...
    [SlashCommand("tagaddauthor", "Add author to the tag")]
    public async Task TagAddAuthor(InteractionContext ctx, [Option("tagname", "Tag that will have author")] string tagName, [Option("authorname", "Pick author of tag")] string authorName)
    {
        //if (!Setup.Permitted(ctx.Guild, Config.ParamType.TagsDefine, ctx)) { Utils.DefaultNotAllowed(ctx); return; }
        Utils.LogUserCommand(ctx);
        try
    ...
    [SlashCommand("tagaddcolor","Add color scheme to tag")]
    public async Task TagColorPicking(InteractionContext ctx, [Option("tagname", "Tag that will have color")] string tagName, [Option("color","PickColor: 0-42 (43)random")] long colorNumber) {
        //if (!Setup.Permitted(ctx.Guild, Config.ParamType.TagsDefine, ctx)) { Utils.DefaultNotAllowed(ctx); return; }
        Utils.LogUserCommand(ctx);
        try
        {
            ...
            else
            {
                ...
                embed.Color = DiscordColor.Red;
                embed.Description = $"Color id: {colorNumber} does not exist. Pick number between: 0-43(43 random)";
                embed.Timestamp = DateTime.Now;
                await ctx.CreateResponseAsync(embed, true);
                return;
            }
    ...
    readonly DiscordColor[] discordColors = { DiscordColor.Aquamarine, DiscordColor.Azure, DiscordColor.Black, DiscordColor.Blue, DiscordColor.Blurple,
            ...
            DiscordColor.Wheat, DiscordColor.Yellow, DiscordColor.White}; //42 colors here
}
